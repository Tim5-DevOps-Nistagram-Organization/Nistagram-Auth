...
if [ "$pullKey" = "0" ]; then
  ...
  SONAR_TOKEN=${token} PULL_KEY=${pullKey} PULL_REQUEST_BRANCH=${branch} BASE=${base} COMPOSE_DOCKER_CLI_BUILD=1 DOCKER_BUILDKIT=1 docker-compose -f ./docker-compose.test.yml  --env-file ./docker-config/.env.test up --build 
fi
...
while ! is_finished nistagram-auth-microservice; do sleep 20; done
# provera Quality Gate-a i da li je neki od testova pao
servers_logs=$(docker logs nistagram-auth --tail 20)
